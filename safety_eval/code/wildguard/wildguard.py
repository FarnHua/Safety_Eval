...
class WildGuardVLLM(WildGuard):
    def __init__(self, batch_size: int = -1, ephemeral_model: bool = True):
        ...
        if ephemeral_model:
            ...
            # self.model = LLM(model=MODEL_NAME, tokenizer_mode="slow", gpu_memory_utilization=0.9, max_model_len=8192, max_num_seqs=8)
            self.model = LLM(model=MODEL_NAME, gpu_memory_utilization=0.9, max_model_len=8192, max_num_seqs=8)
